"""
ecos-reader 상수 정의

ECOS API 통계코드 및 항목코드를 정의합니다.
"""

from __future__ import annotations

# ============================================================================
# 주기 코드
# ============================================================================

PERIOD_DAILY = "D"
PERIOD_MONTHLY = "M"
PERIOD_QUARTERLY = "Q"
PERIOD_ANNUAL = "A"


# ============================================================================
# 금리 지표 (interest_rate)
# ============================================================================

# 한국은행 기준금리
STAT_BASE_RATE = "722Y001"
ITEM_BASE_RATE = "0101000"

# 시장금리 (국고채, 회사채 등)
STAT_MARKET_RATE = "817Y002"

# 국고채 만기별 항목코드
TREASURY_YIELD_ITEMS: dict[str, str] = {
    "1Y": "010190000",  # 국고채 1년
    "3Y": "010200000",  # 국고채 3년
    "5Y": "010200001",  # 국고채 5년
    "10Y": "010210000",  # 국고채 10년
    "20Y": "010220000",  # 국고채 20년
    "30Y": "010230000",  # 국고채 30년
}


# ============================================================================
# 물가 지표 (prices)
# ============================================================================

# 소비자물가지수 (전년동월비)
STAT_CPI = "901Y009"
ITEM_CPI_TOTAL = "0"  # 총지수

# 근원 소비자물가지수 (식료품 및 에너지 제외)
# (#133) item "00"은 총지수(=헤드라인 CPI)라 get_cpi와 동일한 값을 반환하는 버그가 있었다.
#         같은 통계표(901Y010 특수분류)의 식료품·에너지 제외 지수는 item "DB"다.
STAT_CORE_CPI = "901Y010"
ITEM_CORE_CPI = "DB"  # 식료품 및 에너지제외 지수

# 생산자물가지수
STAT_PPI = "404Y014"
ITEM_PPI_TOTAL = "*AA"  # 총지수


# ============================================================================
# 성장 지표 (growth)
# ============================================================================

# GDP (국내총생산)
STAT_GDP_REAL = "200Y110"  # 실질 GDP (원계열, 분기 및 연간)
STAT_GDP_NOMINAL = "200Y109"  # 명목 GDP (원계열, 분기 및 연간)
ITEM_GDP = "10601"  # 국내총생산에 대한 지출

# GDP 디플레이터
STAT_GDP_DEFLATOR = "200Y112"  # 국내총생산에 대한 지출 디플레이터(분기 및 연간)
ITEM_GDP_DEFLATOR = "10601"  # 국내총생산에 대한 지출


# ============================================================================
# 통화 지표 (money)
# ============================================================================

# 통화량 - 각 지표마다 다른 stat code 사용
# M1: 161Y004, M2: 161Y008, Lf: 171Y002
MONEY_SUPPLY_STAT_CODES: dict[str, str] = {
    "M1": "161Y004",  # M1 상품별 구성내역(말잔, 원계열)
    "M2": "161Y008",  # M2 상품별 구성내역(말잔, 원계열)
    "Lf": "171Y002",  # Lf 상품별 구성내역(말잔, 원계열)
}

# 통화량 항목코드
MONEY_SUPPLY_ITEMS: dict[str, str] = {
    "M1": "BBKA00",  # M1 (말잔, 원계열)
    "M2": "BBGA00",  # M2 (말잔, 원계열)
    "Lf": "LE00000",  # Lf(금융기관유동성) : 상품별(말잔, 원계열)
}

# 은행 대출
# 참고: 가계대출은 151Y002, 기업대출은 별도 통계표 필요
STAT_BANK_LENDING = "104Y016"  # 예금은행 대출금(말잔)
STAT_HOUSEHOLD_LENDING = "151Y002"  # 예금취급기관 가계대출(업권별, 월)

# 대출 부문별 항목코드
BANK_LENDING_ITEMS: dict[str, str] = {
    "all": "BDCA1",  # 총대출금
    # Note: household/corporate는 단순 item_code로 구분 불가
    # 가계대출은 151Y002 / 1110000 (예금취급기관)을 사용해야 함
    # 기업대출은 산업별대출금 (131Y016) 등 별도 통계표 필요
}


# ============================================================================
# 환율 지표 (forex)
# ============================================================================

# 731Y001 = 주요국 통화의 대원화환율(일별, 매매기준율). 다통화 일별 종가의 정본.
# (이전 731Y003 은 "원화의 대미달러" 장중 OHLC 전용이라 다통화 item 이 없었음 — #70/#67.)
STAT_EXCHANGE_RATE = "731Y001"

# 통화별 항목코드 (731Y001 기준, 라이브 검증)
EXCHANGE_RATE_ITEMS: dict[str, str] = {
    "USD": "0000001",  # 원/미국달러(매매기준율)
    "JPY": "0000002",  # 원/일본엔(100엔) — 100엔당
    "EUR": "0000003",  # 원/유로
    "CNY": "0000053",  # 원/위안(매매기준율), 2016-01-04~
}

# 실효환율(NEER/REER): ECOS 731 계열에 해당 통계표가 없음(이전 731Y004 는 "주요국 통화의
# 대원화환율(월별)" 로 실효환율이 아니었음 — #71/#67). 올바른 원천 확인 후 재도입할 것.
# (STAT_EFFECTIVE_RATE / ITEM_NEER / ITEM_REER 스캐폴딩 제거)


# ============================================================================
# 국제수지 지표 (bop)
# ============================================================================

# 301Y013 = 2.5.1.1. 국제수지 (경상·자본·금융계정 전체).
# 이전 301Y017 은 "경상수지(계절조정)" 표라 자본/금융계정이 없고 item CA/KA 도 부재했음 (#74/#67).
STAT_BOP = "301Y013"
ITEM_CURRENT_ACCOUNT = "000000"  # 경상수지
ITEM_CAPITAL_ACCOUNT = "BOPC00000000"  # 자본수지
ITEM_FINANCIAL_ACCOUNT = "BOPF00000000"  # 금융계정

# account 선택자 → 항목코드 (301Y013, A/M/Q 주기 지원, 단위 백만달러). 라이브 검증.
BOP_ACCOUNT_ITEMS: dict[str, str] = {
    "current": ITEM_CURRENT_ACCOUNT,  # 경상수지
    "capital": ITEM_CAPITAL_ACCOUNT,  # 자본수지
    "financial": ITEM_FINANCIAL_ACCOUNT,  # 금융계정
}


# ============================================================================
# 실물 지표 (real_economy)
# ============================================================================

# 산업생산지수 (901Y033 = 전산업생산지수). item_code1=A00(농림어업 제외),
# item_code2 로 계열 선택(원계열/계절조정) — 2-축 구조. 라이브 검증.
STAT_INDUSTRIAL_PRODUCTION = "901Y033"
ITEM_INDUSTRIAL_PRODUCTION = "A00"
ITEM_INDUSTRIAL_ORIGINAL = "1"  # 원계열
ITEM_INDUSTRIAL_SEASONAL = "2"  # 계절조정

# 설비투자지수 (901Y066 = 8.1.3. 설비투자지수, 월).
# 이전 901Y049 는 ECOS 에 존재하지 않아 빈 응답을 반환했음 (#72/#67).
STAT_FACILITY_INVESTMENT = "901Y066"
ITEM_FACILITY_INVESTMENT = "I15A"  # 원지수
ITEM_FACILITY_INVESTMENT_SA = "I15B"  # 계절조정지수

# 소매판매액지수 (901Y100 = 재별 및 상품군별 판매액지수). 라이브 검증.
# 정정: 기존 901Y037은 ECOS에서 "건축허가현황"이라 소매판매가 아니었음(오라벨, #127 후속).
# 2-축(상품군 G* × 계열)이라 헤드라인을 총지수(G0) × 계열(item_code2)로 고정한다.
STAT_RETAIL_SALES = "901Y100"
ITEM_RETAIL_SALES = "G0"  # 총지수
RETAIL_SALES_INDEX_ITEMS: dict[str, str] = {
    "nominal": "T1",  # 경상지수
    "real": "T2",  # 불변지수
    "seasonal": "T3",  # 계절조정지수
}

# 경기종합지수 (901Y067, 월, 2020=100). 선행/동행/후행 종합지수. 라이브 검증.
STAT_COMPOSITE_INDEX = "901Y067"
COMPOSITE_INDEX_ITEMS: dict[str, str] = {
    "leading": "I16A",  # 선행종합지수
    "coincident": "I16B",  # 동행종합지수
    "lagging": "I16C",  # 후행종합지수
}

# 무역 (수출입 총괄, 901Y118, 월/연, 천불). 라이브 검증.
STAT_TRADE = "901Y118"
TRADE_FLOW_ITEMS: dict[str, str] = {
    "export": "T002",  # 수출금액
    "import": "T004",  # 수입금액
}


# ============================================================================
# 심리 지표 (sentiment)
# ============================================================================

# 기업경기실사지수 (BSI) — 512Y014(기업경기조사, 전망).
# item_code1 은 업종 선택자(이전 A001/B001/C001 은 존재하지 않던 코드 — #73/#67).
# 설문항목(업황/매출/생산 등)은 item_code2(BA/BB/…)로 별도 선택한다(2-축 구조).
STAT_BSI = "512Y014"
ITEM_BSI_MANUFACTURING = "C0000"  # 제조업
ITEM_BSI_NON_MANUFACTURING = "Y9900"  # 비제조업
ITEM_BSI_ALL = "99988"  # 전산업

# 헤드라인 설문항목(item_code2): 업황전망BSI. 라이브 검증(섹터별 단일 시계열).
ITEM_BSI_OUTLOOK = "BA"  # 업황전망BSI

# sector 선택자 → item_code1 (512Y014). 라이브 검증(전 부문 업황전망 BA 존재 확인, #154).
BSI_SECTOR_ITEMS: dict[str, str] = {
    "all": ITEM_BSI_ALL,  # 전산업 99988
    "manufacturing": ITEM_BSI_MANUFACTURING,  # 제조업 C0000
    "heavy_chemical": "X3000",  # 중화학공업
    "light": "X4000",  # 경공업
    "large": "X5000",  # 대기업
    "sme": "X6000",  # 중소기업
    "export": "X8000",  # 수출기업
    "domestic": "X9000",  # 내수기업
    "non_manufacturing": ITEM_BSI_NON_MANUFACTURING,  # 비제조업 Y9900
    "service": "Y9950",  # 서비스업
}

# 소비자심리지수 (CSI)
STAT_CSI = "511Y002"
ITEM_CSI = "FME"


# ============================================================================
# 재정 지표 (fiscal) - 신규 구현
# ============================================================================

# 통합재정수지
STAT_FISCAL_BALANCE = "901Y013"
ITEM_FISCAL_BALANCE = "A"  # 총수입 (통합재정수지 데이터)


# ============================================================================
# 주식시장 지표 (stock) - 신규 구현
# ============================================================================

# 주가지수
STAT_STOCK_DAILY = "802Y001"  # 주식시장 (일별)
STAT_STOCK_MONTHLY = "901Y014"  # 주식시장 (월/연)

# 주가지수 기본 item_code1 (sub_category 미지정 시 반환할 "지수"). #60 재설계.
#   일별(802Y001): 0001000 KOSPI지수
#   월별(901Y014): 1070000 KOSPI_종가 (이전 1010000=회사수는 오류였음)
ITEM_STOCK_INDEX_DAILY = "0001000"
ITEM_STOCK_INDEX_MONTHLY = "1070000"

# 투자자별 주식거래 (901Y055) — #60 재설계.
# 2차원 구조: item_code1 = 행위(매도/매수/순매수) × 투자자, item_code2 = 지표(거래대금/거래량).
#   item_code1 prefix: S22A 매도 / S22B 매수 / S22C 순매수
#     (prefix + 투자자코드: A=기관 B=개인 C=외국인 D=기타외국인 F=기타법인,
#      prefix 정확일치는 전체 합계행이므로 long-format에서 제외)
#   item_code2: VA 거래대금 / VO 거래량
STAT_INVESTOR_TRADING = "901Y055"
INVESTOR_TRADING_ACTION_PREFIX: dict[str, str] = {
    "매도": "S22A",
    "매수": "S22B",
    "순매수": "S22C",
}
INVESTOR_TRADING_METRIC_CODE: dict[str, str] = {
    "거래대금": "VA",
    "거래량": "VO",
}


# ============================================================================
# 채권시장 지표 (bond) - 신규 구현
# ============================================================================

# 채권 수익률
STAT_BOND_YIELD_TYPE = "901Y015"  # 채권종류별 거래
STAT_BOND_MARKET = "901Y120"  # 채권시장별 거래

# get_bond_market 의 2축 처리(#63). 두 통계표는 2차원이며 축 위치가 다르다:
#   종류별(901Y015): item_code1=채권종류(분류축), item_code2=measure
#   시장별(901Y120): item_code1=measure, item_code2=시장(분류축)
# measure로 한 차원을 고정한 뒤 나머지 분류축을 long-format으로 분류한다.
BOND_YIELD_TYPE_MEASURE_CODE: dict[str, str] = {  # 종류별 → item_code2
    "상장종목수": "2010000",
    "상장잔액": "2020000",
    "거래량": "2030000",
    "거래대금": "2040000",
}
BOND_MARKET_MEASURE_CODE: dict[str, str] = {  # 시장별 → item_code1
    "거래대금": "AMT",
    "거래량": "VOL",
}


# ============================================================================
# M1/M2 세부 변형 (통화 지표 확장)
# ============================================================================

# M1 세부 변형 (평잔/말잔, 계절조정/원계열)
M1_VARIANTS: dict[str, str] = {
    "평잔_계절조정": "161Y001",  # M1 상품별 구성내역(평잔, 계절조정)
    "평잔_원계열": "161Y002",  # M1 상품별 구성내역(평잔, 원계열)
    "말잔_계절조정": "161Y003",  # M1 상품별 구성내역(말잔, 계절조정)
}

M1_ITEMS: dict[str, str] = {
    "평잔_계절조정": "BBLS00",  # M1 (평잔, 계절조정계열)
    "평잔_원계열": "BBLA00",  # M1 (평잔, 원계열)
    "말잔_계절조정": "BBKS00",  # M1 (말잔, 계절조정계열)
}

# M2 세부 변형 (평잔/말잔, 계절조정/원계열)
M2_VARIANTS: dict[str, str] = {
    "평잔_계절조정": "161Y005",  # M2 상품별 구성내역(평잔, 계절조정)
    "평잔_원계열": "161Y006",  # M2 상품별 구성내역(평잔, 원계열)
    "말잔_계절조정": "161Y007",  # M2 상품별 구성내역(말잔, 계절조정)
}

M2_ITEMS: dict[str, str] = {
    "평잔_계절조정": "BBHS00",  # M2 (평잔, 계절조정계열)
    "평잔_원계열": "BBHA00",  # M2 (평잔, 원계열)
    "말잔_계절조정": "BBGS00",  # M2 (말잔, 계절조정계열)
}

# M2 경제주체별
M2_HOLDER_VARIANTS: dict[str, str] = {
    "평잔_계절조정": "161Y009",  # M2 보유주체별(평잔, 계절조정)
    "평잔_원계열": "161Y010",  # M2 보유주체별(평잔, 원계열)
    "말잔_계절조정": "161Y011",  # M2 보유주체별(말잔, 계절조정)
    "말잔_원계열": "161Y012",  # M2 보유주체별(말잔, 원계열)
}

M2_HOLDER_ITEMS: dict[str, str] = {
    "평잔_계절조정": "BBHS00",  # M2(평잔, 계절조정계열)
    "평잔_원계열": "BBHA00",  # M2(평잔, 원계열)
    "말잔_계절조정": "BBGS00",  # M2(말잔, 계절조정계열) — 161Y011의 실제 item_code
    "말잔_원계열": "BBGA00",  # M2(말잔, 원계열) — 161Y012의 실제 item_code
}


# ============================================================================
# 가계신용 지표 확장
# ============================================================================

# 가계신용 (분기)
STAT_HOUSEHOLD_CREDIT_SECTOR = "151Y001"  # 가계신용 (업권별, 분기)
STAT_HOUSEHOLD_CREDIT_PURPOSE = "151Y004"  # 가계신용 (용도별, 분기)

# 예금취급기관 가계대출 (월)
STAT_HOUSEHOLD_LENDING_PURPOSE = "151Y005"  # 예금취급기관 가계대출 (용도별, 월)

# 차주별 가계대출 (분기) — v0.2.0 재설계 (#29)
#
# 이전(v0.1.x) 매핑은 분류축마다 별도 stat_code(181Y001~016)가 있다고 가정했으나
# 실제 ECOS 구조는 stat_code가 2개뿐이고 모든 분류축은 item_code1 prefix로 표현된다:
#   - 181Y001: 차주별 가계대출 신규취급액
#   - 181Y002: 차주별 가계대출 잔액
BORROWER_LOAN_STAT_CODES: dict[str, str] = {
    "신규": "181Y001",
    "잔액": "181Y002",
}

# 분류축 → item_code1 prefix.
#   "전체"는 단일 코드("0000")이고, 나머지는 prefix로 시작하는 모든 item을 묶는다.
#   (prefix는 라이브 ECOS StatisticItemList로 검증 — #29)
#     B0xx 성별 / C0xx 연령대 / D0xx 지역 / E0xx 업권(은행/비은행) /
#     F0xx 담보유형 / G0xx 다중대출 건수
BORROWER_LOAN_CATEGORY_PREFIX: dict[str, str] = {
    "전체": "0000",
    "성별": "B",
    "연령별": "C",
    "지역별": "D",
    "업권별": "E",
    "담보유형별": "F",
    "다중대출별": "G",
}


# ============================================================================
# 금리 지표 확장 (예금/대출 금리)
# ============================================================================

# 예금은행 수신금리
STAT_DEPOSIT_RATE_NEW = "121Y002"  # 예금은행 수신금리 (신규취급액 기준)
STAT_DEPOSIT_RATE_BALANCE = "121Y013"  # 예금은행 수신금리 (잔액 기준)

# 예금은행 대출금리
STAT_LENDING_RATE_NEW = "121Y006"  # 예금은행 대출금리 (신규취급액 기준)
STAT_LENDING_RATE_BALANCE = "121Y015"  # 예금은행 대출금리 (잔액 기준)


# ============================================================================
# GDP 세부 지표 (성장 지표 확장)
# ============================================================================

# 실질 GDP 성장률 — 902Y015 (국제 주요국 경제성장률, OECD 출처), 한국=item "KOR", 단위 %.
# 분기는 전기대비(QoQ, 계절조정), 연간은 전년대비(YoY) 성장률을 동일 통계표에서 제공한다.
# (#134) 과거 매핑 200Y104/200Y106(경제활동별 GDP)은 전 항목이 금액 레벨(십억원)이라
#         성장률(%) 항목이 존재하지 않았고, item "1101"(농림어업)의 GDP 금액을 그대로 반환하는
#         버그가 있었다.
STAT_GDP_GROWTH_RATE = "902Y015"
ITEM_GDP_GROWTH_RATE = "KOR"  # 한국

# GDP 산업별/경제활동별 (계절조정 및 원계열, 명목/실질)
GDP_BY_INDUSTRY_VARIANTS: dict[str, str] = {
    "계절조정_명목": "200Y103",  # 경제활동별 GDP (계절조정, 명목, 분기)
    "계절조정_실질": "200Y104",  # 경제활동별 GDP (계절조정, 실질, 분기)
    "원계열_명목": "200Y105",  # 경제활동별 GDP (원계열, 명목, 분기/연간)
    "원계열_실질": "200Y106",  # 경제활동별 GDP (원계열, 실질, 분기/연간)
}

# GDP 지출항목별 (계절조정+분기 또는 원계열+분기/연간)
# 계절조정 시리즈는 ECOS에서 분기만 제공 — 연간 조회 시 원계열로 자동 fallback.
GDP_BY_EXPENDITURE_VARIANTS: dict[str, str] = {
    "계절조정_명목": "200Y107",  # 지출항목별 GDP (계절조정, 명목, 분기)
    "계절조정_실질": "200Y108",  # 지출항목별 GDP (계절조정, 실질, 분기)
    "원계열_명목": "200Y109",  # 지출항목별 GDP (원계열, 명목, 분기/연간)
    "원계열_실질": "200Y110",  # 지출항목별 GDP (원계열, 실질, 분기/연간)
}

# GDP 디플레이터 (경제활동별)
STAT_GDP_DEFLATOR_BY_INDUSTRY = "200Y111"  # 경제활동별 GDP 디플레이터 (분기/연간)


# ============================================================================
# 물가 지표 확장 (CPI 세부)
# ============================================================================

# CPI 월별 원지수
# 901Y001은 ECOS API에 존재하지 않아 v0.1.6까지 빈 응답을 반환했음 (#32).
# 소비자물가지수(901Y009)를 사용. 레지스트리 cpi_monthly는 총지수(item "0")만,
# get_cpi_monthly()는 전체 COICOP 품목을 반환한다 (v0.3.0 #61).
STAT_CPI_MONTHLY = "901Y009"  # 소비자물가지수

# CPI 세부 항목별 — (stat_code, item_code1) 튜플.
# 이전 v0.1.5 매핑(901Y001~008)은 실재하지 않는 stat_code로 모든 호출이 빈 응답을 반환했음.
# v0.1.6에서 실 ECOS API와 일치하도록 재매핑.
# - 특수분류(전체/상품/서비스/제외 시리즈): 901Y010 (소비자물가지수 특수분류)
# - COICOP 1단계(식료품/주거/교통 등): 901Y009 (소비자물가지수)
CPI_CATEGORY_CODES: dict[str, tuple[str, str]] = {
    "전체": ("901Y010", "00"),  # 총지수 (특수분류)
    "상품": ("901Y010", "21"),  # 상품
    "서비스": ("901Y010", "22"),  # 서비스
    "식품_에너지제외": ("901Y010", "DB"),  # 식료품 및 에너지제외 지수
    "농산물_석유제외": ("901Y010", "QB"),  # 농산물및석유류제외지수
    "식료품_비주류음료": ("901Y009", "A"),  # COICOP A
    "주거_수도_전기": ("901Y009", "D"),  # COICOP D: 주택, 수도, 전기 및 연료
    "교통": ("901Y009", "G"),  # COICOP G
}
